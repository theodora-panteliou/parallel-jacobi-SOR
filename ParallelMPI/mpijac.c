...
static inline double one_jacobi_itteration(double xStart, double yStart,int ystart,int xstart,int maxXCount, int maxYCount,double *u,double *u_old,double deltaX, double deltaY,double cc,double cx,double cy,double relax,double alpha,int prows,int pcolumns)
{
	double updateVal;
	double error2=0.0;
	double fX,fY;
	...
	fY =  yStart;
    for (int y = ystart; y < maxYCount; y++)
    {
       	fX = xStart;
       	for (int x = xstart; x < maxXCount; x++)
       	{
...
int main(int argc, char **argv)
{
	...
	*/
    ...
    for (iterationCount = 0; iterationCount < maxIterationCount; iterationCount++)
    {
	    ...
	    //INSIDE
        loop_error=one_jacobi_itteration(deltaY+yStart,deltaX+xStart,2,2,(pcolumns-2),(prows-2),u,u_old,deltaX,deltaY,cc,cx,cy,relax,alpha,prows,pcolumns);
